...
const Post = () => {
    ...
    useEffect(() => {
        axiosAuthClient.get('/Post/43')
            .then((response: any) => {
                try {
                    setContentValue(HTMLReactParser(new QuillDeltaToHtmlConverter(JSON.parse(response.content).ops, {}).convert()))
                } catch (error) {
                    setContentValue(HTMLReactParser(response.content))
                }
                try {
                    setTitleValue(HTMLReactParser(new QuillDeltaToHtmlConverter(JSON.parse(response.title).ops, {}).convert()))
                } catch (error) {
                    setTitleValue(HTMLReactParser(response.title))
                }
